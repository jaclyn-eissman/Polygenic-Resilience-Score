...
for(i in 1:length(vars)){
  ...
  output <- lm(MEM ~ Age_bl + as.factor(Sex) + PRS, data=Data_bl, na.action=na.omit)
  summary(output)
  ...
  predictor <- vars[i]
  model <- "Main Effects: Cross-Sectional Memory"
...
#Write out
write.xlsx(Results,paste0(dir,"Tables/LM_Main_All_Thresh_without_APOE.xlsx"))
